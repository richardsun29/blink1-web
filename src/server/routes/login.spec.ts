...
describe('Login', () => {
  ...
  function assertIsLoginPage(done: Mocha.Done): request.CallbackHandler {
    return (err, res) => {
      if (err) {
        done(err);
      } else {
  ...
  function assertIsMainPage(done: Mocha.Done): request.CallbackHandler {
    return (err, res) => {
      if (err) {
        done(err);
      } else {
